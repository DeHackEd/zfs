...
static void print_alerts() {
	...
	int errors = alerts.read_errors + alerts.write_errors
	    + alerts.checksum_errors;
	if ((errors) > 1) {
	...
	if (alerts.vdev_checksum_errors) {
		/*
		 * Vdev checksum errors are just warnings because copies=2 can
		 * protect us from data loss. But if that fails or copies=1
		 * then it will also generate other alerts.
		 */
	...
	if (alerts.other_problems)
		alert("Other/unknown issues requiring attention",
		    (p_warning) ? 1 : 2);
	if (use_clear)
	...
	if (alerts.scrubbing || alerts.rebuilding) {
		...
		if (alerts.percent >= 100.00)
			sprintf(buffer,
			    "%s in progress (exceeded 100%%, almost finished)",
			    alerts.rebuilding ? "Rebuild" : "Scrub");
		else
			sprintf(buffer,
			    "%s in progress (%.02f%%, eta %dd %dh %dm)",
			    alerts.rebuilding ? "Rebuild" : "Scrub",
			    alerts.percent * 100.00, days, hours, minutes);
 ...
 */
static void recurse_status_array(zpool_handle_t *zhp, nvlist_t **nv,
    int numelements) {
	int i;
	vdev_stat_t *vs;
	uint_t arraysize;
	...
	for (i = 0; i < numelements; i++) {
		...
		if (currentstate == PSTATE_CACHE) {
			...
			/*
			 * If not a cache, then IO errors on this
			 * disk is quite bad.
			 */
			if (vs->vs_checksum_errors > 0 ||
...
static void recurse_status_config(zpool_handle_t *zhp, nvlist_t *nv) {
	...
	else if (strcmp(string, VDEV_TYPE_RAIDZ) == 0 ||
	    strcmp(string, VDEV_TYPE_MIRROR) == 0)
...
nagios_callback(zpool_handle_t *zhp, void *data)
{
	...
	if (config != NULL) {
		...
		if (ps && ps->pss_func != POOL_SCAN_NONE &&
			 ...
			 * Global shows when the thing started, ended, and data
			 * to process. Local stats get cleared/recalculated on
			 * pool load/import. Global stats look good to users,
			 * but local stats are needed for time calculations to
			 * be remotely accurate - otherwise while a pool is
			 * down/exported it's considered scanning at
			 * 0 bytes/sec.
			 */
		...
		/*
		 * Read the main pool configuration,
...
zpool_do_nagios(int argc, char **argv)
{
	...
	/*
	 * TODO:
	 * Support a pool list on the commandline, an error if any specified
