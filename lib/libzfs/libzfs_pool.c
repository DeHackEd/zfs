    ...
    int indent)
{
	...
	for (c = 0; c < children; c++) {
		vname = zpool_vdev_name(hdl, NULL, child[c], B_TRUE, B_FALSE);
    ...
    const char *old_disk, const char *new_disk, nvlist_t *nvroot, int replacing)
{
	...
	if ((newname = zpool_vdev_name(NULL, NULL, child[0], B_FALSE, B_FALSE)) == NULL)
		return (-1);
    ...
    nvlist_t **schild, uint_t schildren)
{
	...
	for (mc = 0; mc < mchildren; mc++) {
		...
		char *mpath = zpool_vdev_name(zhp->zpool_hdl, zhp,
		    mchild[mc], B_FALSE, B_FALSE);

		for (sc = 0; sc < schildren; sc++) {
			char *spath = zpool_vdev_name(zhp->zpool_hdl, zhp,
			    schild[sc], B_FALSE, B_FALSE);
...
zpool_vdev_name(libzfs_handle_t *hdl, zpool_handle_t *zhp, nvlist_t *nv,
    boolean_t verbose, boolean_t guid)
{
